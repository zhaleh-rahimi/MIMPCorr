...
def fit_tft(train, test, p, q):
    """
    Fits a Temporal Fusion Transformer for multivariate one-step-ahead forecasting (like VARMA extend).
    ...
    # Convert to long format (TFT expects one target column)
    def wide_to_long(df):
    ...
    # Train model
    trainer = torch.optim.Adam(tft.parameters(), lr=1e-3)
    tft.fit(train_loader, max_epochs=5, verbose=False)
    ...
    for series in train.columns:
        ...
        preds = tft.predict(series_loader, mode="prediction").squeeze(-1).detach().numpy()
    ...
    for t in range(len(test)):
        # Add available test data to history
        ...
        preds = tft.predict(pred_loader, mode="prediction").squeeze(-1).detach().numpy()
        preds_next = preds[-len(train.columns):]  # last step for each series
